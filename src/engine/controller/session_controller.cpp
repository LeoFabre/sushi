    ...
    for (auto& track : tracks)
    {
        ...
        switch (track.type)
        {
            ...
            case control::TrackType::REGULAR:
                if (track.buses > 1)
                {
                    std::tie(status, track_id) = _engine->create_multibus_track(track.name, track.buses, std::optional<int>());
                }
                else
                {
                    std::tie(status, track_id) = _engine->create_track(track.name, track.channels, std::optional<int>());
                }
                break;
